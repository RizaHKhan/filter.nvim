...
function M.open_filter(opts)
  ...
  -- Position cursor on filter line right after the "."
  -- Filter line format: "JQ Filter: ." or "YQ Filter: ."
  -- We want cursor right after the "." in insert mode
  -- find() returns 1-indexed position, but nvim_win_set_cursor uses 0-indexed columns
  -- So if "." is at position 12 (1-indexed), we use 12 (0-indexed) to be after it
  local dot_pos = filter_line:find("%.")
  vim.api.nvim_win_set_cursor(0, { 1, dot_pos }) -- dot_pos is 1-indexed, so this puts us after the dot
  vim.cmd("startinsert")
